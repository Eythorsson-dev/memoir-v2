...
describe('BlockHistory', () => {
  ...
  describe('add', () => {
    it('after add, canUndo is true', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      h.add([new BlockDataChanged('a', new Text('hello', []))], null, null)
    ...
    it('events reflects added changes', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      const change = new BlockDataChanged('a', new Text('hello', []))
    ...
    it('multiple adds accumulate in events', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      const c1 = new BlockDataChanged('a', new Text('hi', []))
      const c2 = new BlockDataChanged('a', new Text('hi world', []))
      h.add([c1], null, null)
      h.add([c2], null, null)
    ...
    it('stores selectionBefore and selectionAfter', () => {
      ...
      const after = sel('a', 5)
      h.add([new BlockDataChanged('a', new Text('hello', []))], before, after)
  ...
  describe('undo', () => {
    ...
    it('undo two steps returns intermediate state', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      h.add([new BlockDataChanged('a', new Text('hi', []))], null, null)
      h.add([new BlockDataChanged('a', new Text('hi world', []))], null, null)
  ...
  describe('redo stack cleared on add after undo', () => {
    it('new add after undo clears redo stack', () => {
      ...
      h.add([new BlockDataChanged('a', new Text('hello', []))], null, null)
      h.undo()
      h.add([new BlockDataChanged('a', new Text('different', []))], null, null)
      expect(h.canRedo()).toBe(false)
    })
    ...
    it('events after add-undo-add reflects new transaction only', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      const c1 = new BlockDataChanged('a', new Text('hello', []))
      h.add([c1], null, null)
      h.undo()
      const c2 = new BlockDataChanged('a', new Text('different', []))
      h.add([c2], null, null)
  ...
  describe('MAX_DEPTH cap', () => {
    it('oldest transaction dropped when exceeding MAX_DEPTH', () => {
      ...
      for (let i = 0; i <= BlockHistory.MAX_DEPTH; i++) {
        h.add([new BlockDataChanged('a', new Text(`step${i}`, []))], null, null)
      }
  ...
  describe('updateOrAdd', () => {
    it('coalesces BlockDataChanged for same block', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('h', [])), null, null)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('he', [])), null, null)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('hel', [])), null, null)
    ...
    it('does not coalesce if last transaction has multiple changes', () => {
      ...
      h.add([
        new BlockDataChanged('a', new Text('x', [])),
        new BlockDataChanged('b', new Text('y', [])),
      ], null, null)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('z', [])), null, null)
    ...
    it('does not coalesce if last transaction targets different block', () => {
      const base = makeBlocks('a', 'b')
      const h = new BlockHistory(base)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('x', [])), null, null)
      h.updateOrAdd('b', new BlockDataChanged('b', new Text('y', [])), null, null)
    ...
    it('creates new entry when no prior transaction', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('hello', [])), null, null)
      expect(h.canUndo()).toBe(true)
    })
    ...
    it('coalescing preserves original selectionBefore and updates selectionAfter', () => {
      ...
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('h', [])), firstBefore, firstAfter)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('he', [])), sel('a', 1), secondAfter)
    ...
    it('coalescing updates selectionAfter to latest value', () => {
      ...
      const firstAfter = sel('a', 1)
      const secondAfter = sel('a', 2)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('h', [])), null, firstAfter)
      h.updateOrAdd('a', new BlockDataChanged('a', new Text('he', [])), null, secondAfter)
      h.undo()
      const { selection: redoSel } = h.redo()
    ...
    it('non-coalescing updateOrAdd uses provided selectionBefore', () => {
      ...
      h.updateOrAdd('b', new BlockDataChanged('b', new Text('y', [])), bBefore, null)
  ...
  describe('events getter', () => {
    it('returns flat list up to pointer', () => {
      const base = makeBlocks('a')
      const h = new BlockHistory(base)
      const c1 = new BlockDataChanged('a', new Text('a', []))
      const c2 = new BlockDataChanged('a', new Text('ab', []))
      const c3 = new BlockDataChanged('a', new Text('abc', []))
      h.add([c1], null, null)
      h.add([c2], null, null)
      h.add([c3], null, null)
      h.undo() // pointer at 2
    ...
    it('multiple changes in one transaction are included', () => {
      const base = makeBlocks('a', 'b')
      const h = new BlockHistory(base)
      const c1 = new BlockDataChanged('a', new Text('x', []))
      const c2 = new BlockDataChanged('b', new Text('y', []))
